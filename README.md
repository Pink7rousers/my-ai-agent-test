# 本地模型调用测试 (LangChain + RAG + LoRA微调 + 联网搜索)
...
- **📚 RAG 库**：基于内部文档（`knowledge.txt`）进行精准回答，有效减少大模型幻觉。
- **🧠 微调模型**：使用 LoRA 技术微调 Qwen2.5-1.5B，深度适配客户流失分析等业务场景。
- **🌐 联网搜索**：当内部知识不足时，自动调用 DuckDuckGo 获取最新信息。
- **🔧 工具调用**：支持算术运算等简单工具。
- **🔄 模型切换**：可在 DeepSeek API 和本地微调模型之间自由切换。
- **🐳 容器化**：提供 Docker 镜像，一键运行，支持 GPU 加速。
